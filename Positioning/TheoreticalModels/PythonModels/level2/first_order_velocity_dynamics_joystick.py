...
"""
Level 2 joystick differential-drive wheelchair simulation.

Same high-level input as the NAV2 version:
    v_cmd, omega_cmd
Joystick/WASD only generates those commands interactively.
"""
...
v0 = 0.0
omega0 = 0.0
WINDOW_TITLE = 'Level 2 First-Order Dynamics — Joystick Simulator'
EXTRA_METRIC_LABELS = ['ω_L cmd', 'ω_R cmd', 'v_dot', 'ω_dot']
...
def model_step(state, extra_state, v_cmd, omega_cmd, dt):
    v_cmd = clamp(apply_deadband(v_cmd, v_deadband), -V_LIMIT_MAX, V_LIMIT_MAX)
    omega_cmd = clamp(apply_deadband(omega_cmd, omega_deadband), -OMEGA_LIMIT_MAX, OMEGA_LIMIT_MAX)
    omega_left, omega_right = body_to_wheel_speeds(v_cmd, omega_cmd, r, b)
    v_dot = clamp((v_cmd - state['v']) / tau_v, -a_max, a_max)
    omega_dot = clamp((omega_cmd - state['omega']) / tau_omega, -alpha_max, alpha_max)
    ...
    return [omega_left, omega_right, v_dot, omega_dot]
