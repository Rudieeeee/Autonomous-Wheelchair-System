...
"""
Level 3 joystick differential-drive wheelchair simulation.

Same high-level input as the NAV2 version:
    v_cmd, omega_cmd
Extra physics: wheel-speed-to-force mapping, damping, acceleration and speed limits.
"""
...
v0 = 0.0
omega0 = 0.0
WINDOW_TITLE = 'Level 3 Physics — Joystick Simulator'
EXTRA_METRIC_LABELS = ['F_L', 'F_R', 'F_net', 'τ_net']
...
def model_step(state, extra_state, v_cmd, omega_cmd, dt):
    ...
    force_net = F_left + F_right - c_v * state['v']
    torque_net = (b / 2.0) * (F_right - F_left) - c_omega * state['omega']
    ...
    return [F_left, F_right, force_net, torque_net]
